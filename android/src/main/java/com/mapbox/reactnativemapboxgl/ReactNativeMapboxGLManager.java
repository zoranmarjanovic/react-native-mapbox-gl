...
public class ReactNativeMapboxGLManager extends SimpleViewManager<MapView> {
    ...
    public MapView createViewInstance(ThemedReactContext context) {
        MapView mv = new MapView(context, "pk.foobar"); // Placeholder access token
        mv.onCreate(null);
        return mv;
    }
    ...
    public void updateView(final MapView view,
        ...
        /* This is not in 2.1.0 but should be in next release
        ...
        if (props.hasKey(PROP_ANNOTATIONS)) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                switch (type) {
                    case "point":
                        latitude = annotation.getArray("coordinates").getDouble(0);
                        longitude = annotation.getArray("coordinates").getDouble(1);
                    ...
                    case "polyline": {
                        ...
                        for (int p = 0; p < coordSize; p++) {
                            latitude = annotation.getArray("coordinates").getArray(p).getDouble(0);
                            longitude = annotation.getArray("coordinates").getArray(p).getDouble(1);
        ...
        if (props.hasKey(PROP_SCROLL_ENABLED)) {
            view.setZoomEnabled(props.getBoolean(PROP_SCROLL_ENABLED, true));
        }
